# https://github.com/ozansener/active_learning_coreset/blob/master/tf_base/src/robust_trainer.py
...
class RobustTrainer(object):
    ...
    def __init__(self, learning_rate_net, learning_rate_adv, device_name):
        ...
        with tf.device(device_name):
            ...
            real_l2_loss = tf.add_n(
                [tf.nn.l2_loss(v) for v in real_net_vars if 'bias' not in v.name]) * real_weight_decay
            ...
            self.adv_loss = tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits(self.adv_out, self.ph_per_image_loss)) + adv_l2_loss
            update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
            with tf.control_dependencies(update_ops):
    ...
    def sample_with_replacement(large_batch, loss_estimates, batch_size, gamma=0.5):
        ...
        uniform_prob_list = np.ones((num_examples, 1)) * uniform_prob
        ...
        choices = np.random.choice(num_examples, batch_size, replace=True, p=bimodal_dist_flat)
    ...
    def learning_step(self, session, gamma, compute_adv_loss, train_adv, train_real):
        ...
        per_im_miss_class = 1.0 - per_im_loss_d.astype(np.float32)
        per_im_loss = per_im_miss_class.reshape((per_im_miss_class.shape[0], 1))

        ce_lab = np.concatenate((per_im_loss, 1.0 - per_im_loss), axis=1)

        if compute_adv_loss:
            adv_sum, loss_estimates = session.run([self.adv_summ, tf.nn.softmax(self.adv_out)],
        ...
        if train_real:
            ...
            e_x = np.exp(unnormalized_prob - np.max(unnormalized_prob))
            prob = e_x / e_x.sum()
    ...
    def active_sample(self, session, data, how_many):
        ...
        num_batch = int(np.ceil(num_images * 1.0 / self.batch_size))
        all_prob = np.zeros(num_images)
        all_cp = np.zeros(num_images)
        all_feat = np.zeros((num_images, 512))
        all_l = np.zeros((num_images, 10))
        for b in range(num_batch):
            ...
            if set_max > set_min:
                ...
                loss_estimates = session.run(tf.nn.softmax(self.adv_out),
                                             feed_dict={self.ph_features: feat_values, self.phase: 0})
                unnormalized_prob = loss_estimates[:, 0]
                # e_x = np.exp(unnormalized_prob - np.max(unnormalized_prob))
                # prob = e_x / e_x.sum()
    ...
    def samp(f, n, gamma):
        ...
        bimodal_dist_flat = bimodal_dist[:, 0]
        bimodal_dist_flat = bimodal_dist_flat / bimodal_dist_flat.sum()
        choices = np.random.choice(num_examples, n, replace=False, p=bimodal_dist_flat)
        return choices
