# https://github.com/ozansener/active_learning_coreset/blob/master/tf_base/src/svhn_data.py
...
def extract_test_data(work_directory, one_hot):
    ...
    images = np.transpose(t_dat['X'], axes=[3, 0, 1, 2]).astype('float32')
...
def extract_train_data(work_directory, one_hot, hold_out_size):
    ...
    _images = np.transpose(t_dat['X'], axes=[3, 0, 1, 2]).astype('float32')
    ...
    images = np.zeros((num_images - hold_out_size, 32, 32, 3))
    labels = np.zeros((num_images - hold_out_size))

    if hold_out_size > 0:
        hold_out_images = np.zeros((hold_out_size, 32, 32, 3))
        hold_out_labels = np.zeros((hold_out_size))
    else:
