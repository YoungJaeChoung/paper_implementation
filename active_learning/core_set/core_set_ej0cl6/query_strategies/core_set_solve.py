# https://github.com/ej0cl6/deep-active-learning/blob/master/query_strategies/core_set_solve.py
...
def solve_fac_loc(xx, yy, subset, n, budget):
    ...
    print
    'gen z', datetime.now() - t_start
    for i in range(n):
        # z_i: is a loss
        z[i] = model.addVar(obj=1, ub=0.0, vtype="B", name="z_{}".format(i))

    print
    'gen x y', datetime.now() - t_start
    m = len(xx)
    for i in range(m):
        if i % 1000000 == 0:
            print
            'gen x y {}/{}'.format(i, m), datetime.now() - t_start
        _x = xx[i]
        _y = yy[i]
    ...
    print
    'gen sum 1', datetime.now() - t_start
    yyy = {}
    for v in range(m):
        if i % 1000000 == 0:
            print
            'gen sum 1 {}/{}'.format(i, m), datetime.now() - t_start
        _x = xx[v]
        _y = yy[v]
        if _x not in yyy:
            yyy[_x] = []
...
while ub - lb > tor:
    ...
    print
    "If it succeeds, new max is:", new_max_d, new_min_d
    for v in viol:
        x[xx[v], yy[v]].UB = 0
